...
export function getAssistantReply(input, { products = [], user = null } = {}) {
  ...
  if (
    ...
    return {
      text: "💳 We accept:\n• Mobile Money (MTN, Telecel, AirtelTigo)\n• Debit & credit cards\n• Pay on delivery\n\nYou'll pick your method at checkout — easy!",
    };
  }
  ...
  if (has("my account")) {
    if (user && user.role === "customer") {
      return {
        text: `👤 You're logged in as ${user.name} (${user.email}). Here's your account:`,
